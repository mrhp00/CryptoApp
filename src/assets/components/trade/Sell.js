...
const Sell = (props) => {
  ...
  const SpecialPrice = () => <div>{"قیمت فروش"}</div>;
    ...
    <>
      <div className={props.backgroundCSS}>
        <Form>
          ...
          <div
            className="d-none d-md-block d-lg-block d-xl-block d-xxl-block change-switch"
            onClick={() => {
              let f = from.slice();
              let t = to.slice();
              setFrom(t);
          ...
          <Form.Group className="d-grid mt-4">
            <Button className="btn btn-warning btn-flat ">فروش</Button>
          </Form.Group>
        </Form>
      </div>
    </>
